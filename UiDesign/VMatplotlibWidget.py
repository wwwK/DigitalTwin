...
class MyMplCanvas(FigureCanvas):
    ...
    def update_figure(self):
        ...
        var=GlobalVar.get_value('V')
        if var !=-1:
            for i in range(3):
                self.V[i]=self.V[i+1]
            self.V[3]=var
        self.axes.plot([0, 1, 2, 3], self.V, 'r')
...
class VMatplotlibWidget(QWidget):
    def __init__(self, parent=None):
        super(VMatplotlibWidget, self).__init__(parent)
        self.initUi()
    ...
    def initUi(self):
        ...
        # self.layout.addWidget(self.mpl_ntb)  # 添加完整的 toolbar


if __name__ == '__main__':
    app = QApplication(sys.argv)
    ui = VMatplotlibWidget()
